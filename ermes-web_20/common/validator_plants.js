...
function Changed_channel(id1, lunghezza) {
    ...
    if ((var_lunghezza == 0) || (var_lunghezza < lunghezza)) {
        ...
        if (var_lunghezza == 0) {
            ...
            $(id).after("<div class=\"text-danger small mt-1\">*6 " + numeric_digit + "</div>");
        }
    }
    switch (id1) {
        case "inputCodice":
...
$("#save_modifica_impianto_new").click(function () {
    ...
    $('#save_modifica_impianto_new').next("div").remove();
    ...
    if ((alarm_primo) || (alarm_primo1) || (alarm_secondo) || (alarm_terzo)) {
        ...
        $(".ModeSendLoad").show();
        $('#save_modifica_impianto_new').val("Saving ....")
        updateList();
...
function identificativo_presente() {
    ...
    alarm_first = true;
    //check_first();
    $("#inputCodice").next("div").remove();
    $("#inputCodice").after("<div class=\"text-danger small mt-1\">*" + impianto_duplicato + "</div>");
    $('#save_modifica_impianto_new').next("div").remove();
    $('#save_modifica_impianto_new').val("Save Error")
    $('#save_modifica_impianto_new').after("<div class=\"text-danger small mt-1\">" + wrong_settings + "</div>");
}
function errorUpdate() {
    $('#save_modifica_impianto_new').next("div").remove();
    $('#save_modifica_impianto_new').val("Save Error")
    $('#save_modifica_impianto_new').after("<div class=\"text-danger small mt-1\">Update Error Try Now</div>");
}