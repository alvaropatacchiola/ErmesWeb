...
function Changed_channel(id1, id2, max_ch, min_ch, myfix) {
    ...
    switch (id1) {
        case "value_dos_check":
            valuedoscheck = var_alarm;
            //check_alarm_clock1();
            break;
...
$("#value_dos_check").keypress(function (evt) {
    return keypress_channel_number_dos(evt);
});
$("#value_dos_check").click(function () {
    //$(this).val("");
    $(this).next('p').remove();
    $("#riga_dos_check").removeClass('error');
    valuedoscheck = false;
...
function check_alarm_dos() {

    if ((valuedoscheck == true) ) {
...
function keypress_channel_number_dos(evt) {
...
$("#save_systemlta_new").click(function () {
    ...
    $('#save_systemlta_new').next('p').remove();
    ...
    Changed_channel('value_dos_check', 'riga_dos_check', 50, 10, 0);
   
    if ((var_clock12) || (var_clock24) || (valuedoscheck) ||(validator_password == false)) {
        $('#save_systemlta_new').next('p').remove();
        ...
        $('#save_systemlta_new').after('<p class="error help-block"><span class="label label-important">' + wrong_settings + '</span></p>');
        submit_status = false;
        error_general = false;
