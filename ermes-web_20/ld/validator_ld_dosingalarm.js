...
$("#save_alarmdosingld_new").click(function () {
    if (enable_send_setpoint == false) {
        result_setpoint_change(user_setpoint_disable);
        return false;
    }
    $('#save_alarmdosingld_new').next('p').remove();
    ...
    if ((ph_alarm_dosing) || (cl_alarm_dosing)) {
        ...
        $('#save_alarmdosingld_new').after('<p class="error help-block"><span class="label label-important">' + wrong_settings + '</span></p>');
        submit_status = false;
        error_general = false;
