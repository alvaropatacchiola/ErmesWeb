Public Class alarm_ldt
        ...
        Private Sub alarm_LoadComplete(sender As Object, e As EventArgs) Handles Me.LoadComplete
            ...
            posiziona_alarm_ldtower(nome_impianto, codice_impianto, id_485_impianto, canale)
            End If
        End Sub
    Public Sub posiziona_alarm_ldtower(ByVal nome_impianto As String, codice_impianto As String, id_485_impianto As String, ByVal canale As String)
        ...
        save_alarm_ldtower_new.OnClientClick = String.Format("salva_dati(""{0}""); return false;",
                    ClientScript.GetPostBackEventReference(save_alarm_ldtower_new, ""))
        ...
        If Val(Mid(alarm_value(0), 3, 1)) Then        'Track Low Conductivity
            cond_low_track.Checked = True
            function_java = function_java + "cond_low_track();"
            value_cond_low_track.Text = Format(Val(Mid(alarm_value(0), 6, 4)), "0000")   'Track Low Conductivity Value
            'track_check(ASPxSpinEdit128, ASPxSpinEdit129)
        End If

        If Val(Mid(alarm_value(0), 4, 1)) Then        'Absolute Low Conductivity
            cond_low_absolute.Checked = True
            function_java = function_java + "cond_low_absolute();"
            value_cond_low_abs.Text = Format(Val(Mid(alarm_value(0), 10, 4)), "0000")   'Absolute Low Conductivity Value
            'absolute_check(ASPxSpinEdit129, ASPxSpinEdit128)
        End If

        If Val(Mid(alarm_value(0), 5, 1)) Then        'Off Low Conductivity
            cond_low_off.Checked = True
            function_java = function_java + "cond_low_off();"
        ...
        If Val(Mid(alarm_value(0), 14, 1)) Then        'Track High Conductivity
            cond_high_track.Checked = True
            value_cond_high_track.Text = Format(Val(Mid(alarm_value(0), 17, 4)), "0000")   'Track High Conductivity Value
            function_java = function_java + "cond_high_track();"
        End If

        If Val(Mid(alarm_value(0), 15, 1)) Then        'Absolute High Conductivity
            cond_high_absolute.Checked = True
            value_cond_high_abs.Text = Format(Val(Mid(alarm_value(0), 21, 4)), "0000")   'Absolute High Conductivity Value
            function_java = function_java + "cond_high_absolute();"
        End If

        If Val(Mid(alarm_value(0), 16, 1)) Then        'Off High Conductivity
            cond_high_off.Checked = True
            function_java = function_java + "cond_high_off();"
        End If

        If Val(Mid(alarm_value(0), 25, 1)) Then        'Chemical Level N.O.
            chemical_levels_out_no.Checked = True
        End If

        If Val(Mid(alarm_value(0), 26, 1)) Then        'Chemical Level N.C.
            chemical_levels_out_nc.Checked = True
        End If

        If Val(Mid(alarm_value(0), 27, 1)) Then        'Bleed Timeout Stop 
            bleed_timeout_stop.Checked = True
        End If

        If Val(Mid(alarm_value(0), 28, 1)) Then        'Bleed Timeout No
            bleed_timeout_no.Checked = True
        End If

        If Val(Mid(alarm_value(0), 29, 1)) Then        'No Flow Yes
            no_flow_stop.Checked = True
        End If

        If Val(Mid(alarm_value(0), 30, 1)) Then        'No Flow No
            no_flow_no.Checked = True
        End If

        If Val(Mid(alarm_value(0), 31, 1)) Then        'No Flow Out Alarm Enable 
            out_alarm_enable.Checked = True
        End If

        If Val(Mid(alarm_value(0), 32, 1)) Then        'No Flow Out Alarm Disable
    ...
    Private Function MakeAlarmString() As String
        ...
        If cond_low_absolute.Checked Then                'low conductivity absolute
            Risultato = Risultato + "1"
        Else
            Risultato = Risultato + "0"
        End If

        If cond_low_off.Checked Then                'low conductivity off
            Risultato = Risultato + "1"
        Else
            Risultato = Risultato + "0"
        End If

        Risultato = Risultato + Format(Val(value_cond_low_track.Text), "0000") + Format(Val(value_cond_low_abs.Text), "0000")

        If cond_high_track.Checked Then                'high conductivity track
            Risultato = Risultato + "1"
        Else
            Risultato = Risultato + "0"
        End If

        If cond_high_absolute.Checked Then                'high conductivity absolute
            Risultato = Risultato + "1"
        Else
            Risultato = Risultato + "0"
        End If

        If cond_high_off.Checked Then                'high conductivity off
            Risultato = Risultato + "1"
        Else
            Risultato = Risultato + "0"
        End If

        Risultato = Risultato + Format(Val(value_cond_high_track.Text), "0000") + Format(Val(value_cond_high_abs.Text), "0000")

        If chemical_levels_out_no.Checked Then            'Chemical levels N.O.
            Risultato = Risultato + "1"
        Else
            Risultato = Risultato + "0"
        End If

        If chemical_levels_out_nc.Checked Then            'Chemical levels N.C.
            Risultato = Risultato + "1"
        Else
            Risultato = Risultato + "0"
        End If

        If bleed_timeout_stop.Checked Then           'Bleed Timeout stop
            Risultato = Risultato + "1"
        Else
            Risultato = Risultato + "0"
        End If

        If bleed_timeout_no.Checked Then            ' Bleed Timeout no
            Risultato = Risultato + "1"
        Else
            Risultato = Risultato + "0"
        End If
        ...
        ''--------------------------------------------------------------------------------------------------------------------------------------------------------
        'If MTower_Type = "Cd_pH_Cl" Or MTower_Type = "Cd_pH_rH" Then
        '    main_function_config.get_tipo_strumento_tower(personalizzazione_aquacare, config_value(0), config_value(1), config_value(2), config_value(3), version_str, , fattore_divisione_temp)
    ...
    Protected Sub save_alarm_ldtower_new_Click(sender As Object, e As EventArgs) Handles save_alarm_ldtower_new.Click
        ...
        Response.Redirect("~/ldtower.aspx?codice=" + codice_impianto + "&id_485=" + id_485_impianto + "&pagina=7" + "&nome_impianto=" + nome_impianto + "&statistica=" + statistica + "&" + url_result)
