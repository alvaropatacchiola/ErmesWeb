...
$("#save_manual_newlds").click(function () {
    ...
    $('#save_manual_newlds').next('p').remove();
    ...
    if ((alarm_temp1) || (alarm_temp2) ||
        (alarm_temp7) || (alarm_temp8) || (one_setting)) {
        $('#save_manual_newlds').next('p').remove();
        $('#save_manual_newlds').after('<p class="error help-block"><span class="label label-important">' + wrong_settings + '</span></p>');
        submit_status = false;
        error_general = false;
        return false;
    }
