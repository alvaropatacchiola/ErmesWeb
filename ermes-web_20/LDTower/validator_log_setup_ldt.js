...
$("#save_logsetup_ldtower_new").click(function () {
    if (enable_send_setpoint == false) {
        result_setpoint_change(user_setpoint_disable);
        return false;
    }

    $('#save_logsetup_ldtower_new').next('p').remove();
    ...
    if ((log_every_alarm_h) || (log_every_alarm_m) || (log_time_alarm_24) || (log_time_alarm_12)) {
        ...
        $('#save_logsetup_ldtower_new').after('<p class="error help-block"><span class="label label-important">' + wrong_settings + '</span></p>');
        submit_status = false;
        error_general = false;
