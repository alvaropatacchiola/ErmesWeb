...
$("#save_international_new_ld4").click(function () {
    if (enable_send_setpoint == false) {
        result_setpoint_change(user_setpoint_disable);
        return false;
    }

    $('#save_international_new_ld4').next('p').remove();
    ...
    if ((var_clock12) || (var_clock24)) {
        ...
        $('#save_international_new_ld4').after('<p class="error help-block"><span class="label label-important">' + wrong_settings + ' </span></p>');
        submit_status = false;
        error_general = false;
        return false;
