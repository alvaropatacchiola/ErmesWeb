...
$("#save_alarmdosingld4_new").click(function () {
    if (enable_send_setpoint == false) {
        result_setpoint_change(user_setpoint_disable);
        return false;
    }

    $('#save_alarmdosingld4_new').next('p').remove();
    if (!($('#ph_alarm_dosing_enable').is(':checked'))) {
        Changed_channel('value_ph_alarm_dosing', 'riga1_ph_alarm_dosing', 99, 0, 0);
    }
    ...
    if ((ph_alarm_dosing) || (cl_alarm_dosing)) {
        ...
        $('#save_alarmdosingld4_new').after('<p class="error help-block"><span class="label label-important">' + wrong_settings + '</span></p>');
        submit_status = false;
        error_general = false;
