...
$("#save_probefailurewd_new").click(function () {
    if (enable_send_setpoint == false) {
        result_setpoint_change(user_setpoint_disable);
        return false;
    }

    $('#save_probefailurewd_new').next('p').remove();
    ...
    if ((ph_alarm_failure) || (cl_alarm_failure)) {
        ...
        $('#save_probefailurewd_new').after('<p class="error help-block"><span class="label label-important">' + wrong_settings + '</span></p>');
        submit_status = false;
        error_general = false;
