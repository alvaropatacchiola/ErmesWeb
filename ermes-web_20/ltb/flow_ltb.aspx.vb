Public Class flow_ltb
    ...
    Private Sub flow_ltb_LoadComplete(sender As Object, e As EventArgs) Handles Me.LoadComplete
        ...
        If Not IsPostBack Or result = "1" Then
            ...
            posiziona_flow_ltb(nome_impianto, codice_impianto, id_485_impianto, canale, GetGlobalResourceObject("ld_global", "ma_output"), GetGlobalResourceObject("ld_global", "out_of_range_alarm"), GetGlobalResourceObject("ld_global", "min_max_high"), GetGlobalResourceObject("ld_global", "min_max_low"), GetGlobalResourceObject("ld_global", "time"))
        End If
    End Sub
    Public Sub posiziona_flow_ltb(ByVal nome_impianto As String, codice_impianto As String, id_485_impianto As String, ByVal canale As String, _
        ...
        save_flowltb_new.OnClientClick = String.Format("salva_dati(""{0}""); return false;", _
                    ClientScript.GetPostBackEventReference(save_flowltb_new, ""))
        ...
        If Val(main_function.get_version(riga_strumento.nome)) < 125 Then
            ...
            enable_dioxide.Visible = False
        Else
            enable_dioxide.Visible = True
        End If
        ...
        If Val(main_function.get_version(riga_strumento.nome)) < 117 Then
            function_java = "disable_allarme();"
        Else

            function_java = "enable_allarme();"
        End If
        ...
        Else
            ...
            Dim soglia_val1 As String = Mid(flow_value(5), 1, 4)
            Dim soglia_val2 As String = Mid(flow_value(6), 1, 4)
            ...
            Dim temp_calc As Single = Val(soglia_val1) / fattore_divisione_temp2
            value_alarm_soglie_max.Text = Replace(temp_calc.ToString(), ",", ".")




            temp_calc = Val(soglia_val2) / fattore_divisione_temp2
            value_alarm_soglie_min.Text = Replace(temp_calc.ToString(), ",", ".")

            

            value_alarm_soglie_time.Text = Val(time_soglia).ToString


            If mode_soglia = "0" Then 'disable
                Dis_soglie.Checked = True 'dis
                function_java = "disable_soglia();"
            End If
            If mode_soglia = "2" Then 'stop
                Stop_soglie.Checked = True 'rev
                function_java = "enable_soglia();"
            End If
            If mode_soglia = "1" Then 'dose
                Dose_soglie.Checked = True 'dir
                function_java = "enable_soglia();"
            End If
        ...
        If Val(main_function.get_version(riga_strumento.nome)) >= 125 Then
            Dim mode_dioxide As String
            Dim value_dioxide As String

            mode_dioxide = Mid(flow_value(7), 1, 1)
            value_dioxide = Mid(flow_value(8), 1, 4)

            Dim temp_calc As Single = Val(value_dioxide) / 100
            value_alarm_dioxide.Text = Replace(temp_calc.ToString(), ",", ".")

            If mode_dioxide = "1" Then 'enable
                en_dioxide.Checked = True 'dir

            End If

            If mode_dioxide = "0" Then 'enable
                dis_dioxide.Checked = True 'dir
    ...
    Private Function MakeFlowString() As String
        ...
        Else
            ...
            If Dis_soglie.Checked = True Then 'disable
                app_mode_soglia = "0"
            End If
            If Dose_soglie.Checked = True Then 'dose
                app_mode_soglia = "1"
            End If
            If Stop_soglie.Checked = True Then 'stop
                app_mode_soglia = "2"
            End If

            app_time_soglia = Format(Val(value_alarm_soglie_time.Text), "00")


            app_soglia_max_i = Val(value_alarm_soglie_max.Text) * fattore_divisione_temp2
            app_soglia_max = Format(app_soglia_max_i, "0000")

            app_soglia_min_i = Val(value_alarm_soglie_min.Text) * fattore_divisione_temp2
            ...
            If Val(main_function.get_version(riga_strumento.nome)) >= 125 Then
                ...
                Dim app_value_dioxide_i As Integer

                If en_dioxide.Checked = True Then 'disable
                    app_mode_dioxide = "1"
                End If

                If dis_dioxide.Checked = True Then 'disable
                    app_mode_dioxide = "0"
                End If



                app_value_dioxide_i = Val(value_alarm_dioxide.Text) * 100
                app_value_dioxide = Format(app_value_dioxide_i, "0000")


                Risultato = app_mode_flow + app_time_flow + app_mode_soglia + app_time_soglia + app_soglia_max + app_soglia_min + app_mode_dioxide + app_value_dioxide
    ...
    Protected Sub save_flowltb_new_Click(sender As Object, e As EventArgs) Handles save_flowltb_new.Click
        ...
        Response.Redirect("~/ltb.aspx?codice=" + codice_impianto + "&id_485=" + id_485_impianto + "&pagina=3" + "&nome_impianto=" + nome_impianto + "&statistica=" + statistica + "&" + url_result)
